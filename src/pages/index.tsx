...
const updateChatbotMessage = (
	...
	const updatedConversation = conversation.reduce(
		(acc: ConversationEntry[], e: ConversationEntry) => [...acc, e.id === interactionId ? { ...e, message: e.message + message.token } : e],
		[]
	);
...
export default function Home() {
	...
	useEffect(() => {
		supabaseBrowserClient.auth.getSession().then(({ data: { session } }) => {
			if (!session) {
				supabaseBrowserClient.auth.onAuthStateChange((_e, newSession) => setUserId(newSession?.user.id));
			} else {
	...
	if (!userId) return <Auth supabaseClient={supabaseBrowserClient} appearance={{ theme: ThemeSupa }} />;
	...
	return (
		<>
			...
			<main className={styles.main}>
				<div style={{ position: 'relative', height: '90vh', overflow: 'hidden', width: '600px', margin: '0 auto' }}>
					<MainContainer>
						<ChatContainer>
							<ConversationHeader>
								<ConversationHeader.Actions></ConversationHeader.Actions>
								<ConversationHeader.Content userName="Langchain Supa GPT" info={statusMessage} />
							</ConversationHeader>
									...
									return (
										...
										>
											<Message.CustomContent>
												<ReactMarkdown remarkPlugins={[remarkMath, rehypeKatex]}>{entry.message}</ReactMarkdown>
											</Message.CustomContent>
											<Message.Footer sentTime={timeago.format(entry.date)} sender={entry.speaker === 'bot' ? 'AI' : 'You'} />
										</Message>
									);
								})}
							</MessageList>
