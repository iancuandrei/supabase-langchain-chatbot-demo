...
const handleRequest = async ({ prompt, userId, supabaseAuthedClient }: { prompt: string; userId: string; supabaseAuthedClient: SupabaseClient }) => {
	try {
		const channel = supabaseAuthedClient.channel(userId);
		const { data } = await supabaseAuthedClient.from('conversations').insert({ speaker: 'ai', user_id: userId }).select().single().throwOnError();
		...
		channel.subscribe(async (status) => {
			if (status === 'SUBSCRIBED') {
				...
				const matches = await getMatchesFromEmbeddings(inquiry, supabaseAuthedClient, 2);
				...
				const chat = new ChatOpenAI({
					...
					callbackManager: CallbackManager.fromHandlers({
						...
						async handleLLMEnd(result) {
							// Store answer in DB
							await supabaseAuthedClient.from('conversations').update({ entry: result.generations[0][0].text }).eq('id', interactionId);
							await channel.send({
								type: 'broadcast',
								event: 'chat',
								payload: {
